...
class ServerProtocol(asyncio.Protocol):
    ...
    def data_received(self, data):
        ...
        if splited[0] == "IAMAT":
            if len(splited) != 4 or '-' not in splited[2] or '+' not in splited[2] or not splited[3].replace('.', '', 1).isdigit() or re.search('[a-zA-Z]', splited[2]) or re.search('[a-zA-Z]', splited[3]):
    ...
    async def handleAT(self, input):
        ...
        if id in clients:
            ...
            clients[id] = [location, time_stamp, og_time]
    ...
    async def handleWHATSAT(self, input):
        ...
        location = re.split(r'[+-]', clients[id][0])
